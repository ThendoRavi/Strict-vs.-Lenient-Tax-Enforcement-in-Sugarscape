turtles-own [
  sugar               ;; amount of sugar this turtle has
  metabolism          ;; sugar burned per tick
  vision              ;; how far the turtle can see in NSEW directions
  vision-points       ;; list of visible patch coordinates
  strategy            ;; "pay", "partial", or "evade"
  tax-due             ;; tax owed
  tax-paid            ;; tax paid this cycle
  compliance-history  ;; record of 'full', 'partial', or 'none'
  is-punished?        ;; true if under punishment
  punishment-timer    ;; ticks left under punishment
]

patches-own [
  psugar              ;; sugar on this patch
  max-psugar          ;; max sugar allowed
]

globals [
  tax-revenue         ;; total tax collected
  audit-percentage    ;; % of agents audited
  punishment-mode     ;; "strict" or "lenient"
]
...
to turtle-setup
  ...
  ;; Assign tax strategy
  let r random 3
  if r = 0 [ set strategy "pay" ]
  if r = 1 [ set strategy "partial" ]
  if r = 2 [ set strategy "evade" ]
...
to handle-tax
  ask turtles [
    ...
    if sugar < 10 [ set tax-due sugar * 0.1 ]
    if sugar >= 10 and sugar < 30 [ set tax-due sugar * 0.2 ]
    if sugar >= 30 and sugar < 50 [ set tax-due sugar * 0.3 ]
    if sugar >= 50 [ set tax-due sugar * 0.4 ]

    ;; Set tax payment
    if strategy = "pay" [ set tax-paid tax-due ]
    if strategy = "partial" [ set tax-paid tax-due * 0.5 ]
    if strategy = "evade" [ set tax-paid 0 ]
...
to audit-and-punish
  let audited-sample n-of (audit-percentage * count turtles) turtles
  ask audited-sample [
    if tax-paid < tax-due [
      if punishment-mode = "strict" [
        set is-punished? true
        set punishment-timer 10
