...
; Patch Utilities
to patch-recolor
  ; Green vegetation theme: Dark green (rich) to white (depleted)
  ; More sugar = darker green, no sugar = white
  set pcolor scale-color green psugar 10 0
end
...
to do-visualization
  ask turtles [
    if visualization-mode = "none" [ set color red ]  ; Changed to red for better contrast with green
    if visualization-mode = "vision" [ set color scale-color orange vision 1 6 ]
    if visualization-mode = "metabolism" [ set color scale-color violet metabolism 1 4 ]
    if visualization-mode = "strategy" [
      ; Color based on last tax compliance behavior from compliance-history
      ; This provides more persistent coloring than just the strategy variable
      ifelse length compliance-history > 0 [
        let last-compliance last compliance-history
        if last-compliance = "full" [ set color magenta ]      ; 🔵 Full compliance
        if last-compliance = "partial" [ set color yellow ] ; 🟡 Partial payment
        if last-compliance = "none" [ set color red ]   ; 🟣 Tax evader
      ] [
